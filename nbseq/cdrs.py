...
def extract_CDRs(seqs, CDRs=None, library=None, push_left=False):
	...
	assert all(len(cdr_pos) == 2 for cdr, cdr_pos in CDRs.items()), 'Error: CDRs must be specified as a dict mapping CDR names to CDR (start, end) positions'
	for subseq_name, (start, end) in CDRs.items():
		CDR_df[subseq_name] = seqs.apply(get_subsequence_extractor(start,end))

	return CDR_df

def identify_clonotype(df, cdrs=['CDR1','CDR2','CDR3'],id_col='clonotypeID',seq_col='clonotype'):
	col_hashes = df[cdrs].apply(lambda c: c.apply(md5_ungapped))
	df[id_col] = col_hashes.apply(lambda row: md5(''.join(row.values.astype(str))), axis=1)
	if seq_col is not None:
		df[seq_col] = df[cdrs].apply(lambda row: ''.join(row.values.astype(str)), axis=1)
	return df
